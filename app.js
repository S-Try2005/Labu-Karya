/* ============================================================
   LABU KARYA PUBLISHER — app.js (Firebase Version)
   
   CARA PAKAI:
   Tambahkan di <head> index.html / halaman utama:
   ...
   (status === 'approved'). Submission baru masuk ke Firestore
   sebagai 'pending' dan hanya tampil setelah admin menyetujuinya.
   ============================================================ */
...
/* ──────────────────────────────────────────────
   2. STATE
   ────────────────────────────────────────────── */
let approvedBooksCache = []; // buku dari Firestore (status approved)
let activeGenre  = 'all';
let searchQuery  = '';
...
function renderBookCard(book, index) {
  let coverHTML = '';
  if (book.coverUrl) {
    // Cover dari Firebase Storage
    coverHTML = `<img src="${book.coverUrl}" alt="Cover ${book.judul}" class="book-cover-img"/>`;
  } else {
    // Fallback warna + emoji (untuk seed books atau jika tidak ada cover)
    const colors = {
  ...
  const ts     = book.submittedAt || book.approvedAt;
  const isNew  = ts && (Date.now() - new Date(ts).getTime()) < 86400000 * 3;
...
function renderCatalog() {
  ...
  // Gabungkan seed books + buku approved dari Firestore
  const allBooks = [...SEED_BOOKS, ...approvedBooksCache];
  if (statBuku) animateCount(statBuku, allBooks.length);
...
/* ──────────────────────────────────────────────
   6. FIREBASE LISTENER — approved books
   ────────────────────────────────────────────── */
function startApprovedListener() {
  db.collection('submissions')
    ...
    .onSnapshot(snapshot => {
      approvedBooksCache = snapshot.docs.map(doc => ({ ...doc.data(), id: doc.id }));
      renderCatalog();
    }, err => {
      console.warn('Firestore listener error (pastikan index sudah dibuat):', err);
      // Fallback: query tanpa orderBy jika index belum dibuat
      db.collection('submissions')
        .where('status', '==', 'approved')
        .onSnapshot(snap => {
          approvedBooksCache = snap.docs.map(d => ({ ...d.data(), id: d.id }));
          renderCatalog();
        });
    });
}
...
/* ──────────────────────────────────────────────
   8. UPLOAD BUTTON
   ────────────────────────────────────────────── */
function redirectToUpload() { window.location.href = 'upload.html'; }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initUploadButtons();
  initFilters();
  renderCatalog();       // Render seed books dulu
  startApprovedListener(); // Lalu pasang listener Firestore (akan re-render otomatis)
});
